...
def evaluate(model, data_iterator, params, mark='Eval', verbose=False):
    ...
    for _ in tqdm(range(params.eval_steps), desc="Evaluation Progress"):
        # fetch the next evaluation batch
        batch_data, batch_tags = next(data_iterator)
        batch_masks = batch_data.gt(0)

        torch.cuda.empty_cache()  # TODO ?????
        output = model(batch_data, token_type_ids=None, attention_mask=batch_masks, labels=batch_tags)
        loss = output.loss
        ...
        # Use the logits from the model output for predictions
        logits = batch_output.logits  # Get the logits for predictions (shape: (batch_size, max_len, num_labels))

        # Detach logits and move to CPU for further processing
        logits = logits.detach().cpu().numpy()
        batch_tags = batch_tags.to('cpu').numpy()

        # Convert predictions and true tags to label names
        pred_tags.extend([idx2tag.get(idx) for indices in np.argmax(logits, axis=2) for idx in indices])
